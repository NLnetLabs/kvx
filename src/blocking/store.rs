...
pub type TransactionCallback = Box<dyn Fn(&dyn KeyValueStoreBackend) -> Result<()>>;
...
impl KeyValueStore {
    pub fn new(storage_uri: &Url, namespace: impl Into<SegmentBuf>) -> Result<KeyValueStore> {
        ...
        let inner: Box<dyn PubKeyValueStoreBackend> = match storage_uri.scheme() {
            ...
            "postgres" => Box::new(crate::blocking::implementations::postgres::Postgres::new(
                storage_uri,
                namespace,
            )?),
